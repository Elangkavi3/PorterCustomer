...
function formatDateLabel(value, notAvailableLabel) {
  if (!value) {
...
function VehicleDetailsScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      async function hydrate() {
        ...
        const normalizedStatus = normalizeVehicleStatus(
          data[KEYS.vehicleSafetyStatus] || 'ROAD_READY',
        );
        ...
        setVehicleInfo({
          ...
          lastInspection: formatDateLabel(lastInspectionRaw, t('vehicle.notAvailable')),
  ...
  return (
    <AppScreen>
      <AppHeader title={t('vehicle.detailsTitle')} subtitle={t('vehicle.detailsSubtitle')} />
        ...
        <AppCard>
          ...
          <View style={{ marginTop: spacing[2], gap: spacing[1] }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={[typography.caption, { color: colors.textSecondary }]}>{t('vehicle.model')}</Text>
              <Text style={[typography.label, { color: colors.textPrimary }]}>{vehicleInfo.vehicleModel}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={[typography.caption, { color: colors.textSecondary }]}>{t('vehicle.insuranceExpiry')}</Text>
              <Text style={[typography.label, { color: colors.textPrimary }]}>{vehicleInfo.insuranceExpiry}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={[typography.caption, { color: colors.textSecondary }]}>{t('vehicle.fcExpiry')}</Text>
              <Text style={[typography.label, { color: colors.textPrimary }]}>{vehicleInfo.fcExpiry}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={[typography.caption, { color: colors.textSecondary }]}>{t('vehicle.fuelLevel')}</Text>
              <Text style={[typography.label, { color: colors.textPrimary }]}>{vehicleInfo.fuelLevel}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={[typography.caption, { color: colors.textSecondary }]}>{t('vehicle.lastInspection')}</Text>
