...
function DocumentsScreen({ navigation }) {
    ...
    useCallback(() => {
      async function loadDocuments() {
        ...
        if (Array.isArray(storedDocuments)) {
          const normalized = storedDocuments.map((doc, index) => ({
            id: String(doc?.key || doc?.name || `document-${index}`) + `-${index}`,
            key: normalizeKey(doc?.key || doc?.name),
            name: doc?.name || (doc?.key ? String(doc.key).replace(/_/g, ' ') : `Document ${index + 1}`),
  ...
  const hasAdharDetails = useMemo(
    () => documents.some(item => item.key === 'adhar_details' || normalizeKey(item.name) === 'adhar_details'),
    [documents],
  );
          ...
          hasAdharDetails
            ...
            : (
              <TouchableOpacity
                ...
                onPress={() =>
                  navigation.navigate('DocumentUploadScreen', {
                    documentName: 'Adhar Details',
                    documentKey: 'adhar_details',
                  })
                }
              >
                <Text style={styles.actionButtonText}>Upload Adhar Details</Text>
              </TouchableOpacity>
            )
        }
